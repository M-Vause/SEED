...
function [vals,vars] = EX05_LorenzTimeDelay(varargin)
...
function [vals,vars] = run()
    ...
    vals = Xi;
    Theta = poolData(x,r,polyorder,usesine);
    vars = poolDataLIST({'u','v','w'},Xi,n,polyorder,usesine);
    for k=1:length(Xi)
...
function [variables,values] = get_params()
    variables = {'polyorder','Lorenz parameter sigma','Lorenz parameter beta','Lorenz parameter rho','Number of Dimensions','Initial Conditions','dt'};
    values = {3,10,'8/3',28,3,'-8; 8; 27',0.001};
end